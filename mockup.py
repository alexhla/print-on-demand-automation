...
ap.add_argument('-es', '--enlarge_styled_images', action='store_true',
	help='pixel size of preview of original image that is overlayed onto final mockup')

ap.add_argument('-ro', '--resize_original', nargs=1, metavar=('RESIZE_ORIGINAL'),
	help='pixel size of preview of original image that is overlayed onto final mockup')

ap.add_argument('-ab', '--add_border', nargs=1, metavar=('ADD_BORDER'),
	help='border size of original image that is overlayed onto final mockup')

ap.add_argument('-pp', '--place_preview', nargs=1, metavar=('PLACE_PREVIEW'),
	help='where the original image preview is to be overlayed on the final mockup (topleft, topmiddle, topright, bottomleft, bottommiddle, bottom right)')

ap.add_argument('-pw', '--place_watermark', nargs=1, metavar=('PLACE_WATERMARK'),
	help='where the watermark overlayed on the final mockup (topleft, topmiddle, topright, bottomleft, bottommiddle, bottom right)')
...
if args['resize_original']:
	for filename in os.listdir(ORIGINAL_IMAGE_FOLDER):
			if 'preview' not in filename:
				...
				pixel_max = int(args['resize_original'][0])
...
if args['place_preview']:
	...
	for filename in os.listdir(STYLED2X_IMAGE_FOLDER):
		...
		if placement == 'topleft':
			...
			x1 = y1 = 0
			x2 = preview_image.shape[0]
			y2 = preview_image.shape[1]
			...
			x2 = styled_img.shape[0]
			y2 = preview_image.shape[1]

		elif placement == 'bottommiddle':
			x1 = y1 = 0
			x2 = preview_image.shape[0]
			y2 = preview_image.shape[1]
		...
		MOCKUP_IMAGE_PATH = os.path.join(MOCKUP_IMAGE_FOLDER, filename)
		cv2.imwrite(MOCKUP_IMAGE_PATH, styled_img)
		print(placement)


if args['place_watermark']:
	for filename in os.listdir(MOCKUP_IMAGE_FOLDER):
		watermark_image = Image.open(WATERMARK_IMAGE_PATH, 'r')
		styled_img = Image.open(os.path.join(MOCKUP_IMAGE_FOLDER, filename), 'r')
		...
		text_img.save(MOCKUP_IMAGE_FOLDER+filename, format="png")
		print(placement)